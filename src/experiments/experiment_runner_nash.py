...
def insert_into_results(database_name, db_result):
    ...
    with conn:
        ...
        cursor.execute('''INSERT INTO Results 
                        (timestamp, model, data_files, utility_vector, max_utility, min_utility, sum_utility, product_utility, solving_runtime, solver, configuration, envy_pairs)
                        VALUES (CURRENT_TIMESTAMP, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                        (db_result["model"], db_result["data_files"], f'{db_result["utilities"]}', db_result["max_utility"],
                        db_result["min_utility"], db_result["sum_utility"], db_result["product_utility"], db_result["solving_runtime"], db_result["solver"], db_result["configuration"], db_result["envy_pairs"]))
...
class ExperimentRunner:
    ...
    def run_experiment(self, experiment : Experiment):
	    ...
	    "product_utility" : self.product(utils), 
...
if __name__ == "__main__":
    ...
    database_name = os.path.join(result_dir, 'nash_results.db')
    create_database(database_name)
    print(f"Database '{database_name}' created successfully.")

    filename =  os.path.join(os.path.dirname(__file__), 'nash_utilitarian.json')    
