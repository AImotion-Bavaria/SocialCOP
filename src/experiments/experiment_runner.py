...
def objective(trial: optuna.Trial, experiment: Experiment) -> float:
    ...
    env = DummyVecEnv([lambda: GiniEnv(grid_size=5, render_mode='console', experiment=experiment, experiment_runner=experiment_runner)])
    ...
    eval_envs = DummyVecEnv([lambda: GiniEnv(grid_size=5, render_mode='console', experiment=experiment, experiment_runner=experiment_runner)])
...
class GiniEnv(gym.Env):
    ...
    def __init__(self, experiment, experiment_runner, grid_size=5, render_mode=None,  ):
       ...
       # simple_agents.add_file("src\\stable_baselines\\temp\\"+str(self.index%3)+"_"+self.start,parse_data=True)
        ...
        with instance.branch() as inst:          
           ...
           # model.add_file('src\\stable_baselines\\temp\\0_generic_preferences.dzn')
            ...
            data_file_path = self.experiment.path + "data\\" + self.experiment.model_inst[1][self.index%3]
