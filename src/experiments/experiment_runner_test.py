...
result_dir = os.path.join('src/experiments/01_07/results')
...
file_dir = "src/experiments/01_07/trained_models_none/trained_mz.zip"
logdir = "/home/ruttmann/projects/SocialCOP/src/experiments/utilitarian/results"
...
def objective(trial: optuna.Trial, experiment: Experiment) -> float:
   ...
   # if experiment.model_names.lower() in ["ppo", "a2c"]:
    ...
    #model = model_class("MultiInputPolicy", , **model_kwargs)
...
class ExperimentRunner:
    ...
    def run_experiment(self, experiment : Experiment):
        ...
        study = optuna.create_study(
            study_name=f"26_06_{experiment.get_identifier_short()}",
            sampler=sampler,
   ...
   # if experiment.model_names.lower() in ["ppo", "a2c"]:
    #    model_kwargs["n_epochs"] = 5
        ...
        model.learn(total_timesteps=10000, callback=TensorboardCallback(), tb_log_name=experiment.get_identifier_short(), progress_bar=True)

            # Save the final model
        final_model_path = os.path.join(models_dir, f"{experiment.get_identifier_short()}_best_model.zip")
        model.save(final_model_path)

        print(f"Final trained model saved at: {os.path.abspath(final_model_path)}")

#final model total_timesteps erhöhen
