...
def add_proportionality_objective(social_mapper, instance : Instance):
    instance.add_string(f"int: m;")
    instance.add_string(f"array[1..{NUM_AGENTS}] of var float: max_values;")
    instance.add_string(f"array[1..{NUM_AGENTS}] of var int: selected;")
    instance.add_string(f"constraint forall (p in 1..{NUM_AGENTS}) (max_values[p] = max([possible_solutions[i, p] | i in 1..m]) / {NUM_AGENTS});")
    instance.add_string(f"constraint forall (p in 1..{NUM_AGENTS}) (selected[p] >= max_values[p]);")
    instance.add_string(f"constraint exists(i in 1..m) (forall(j in 1..{NUM_AGENTS}) (selected[j] = possible_solutions[i, j]));")
