...
models_dir = "src/stable-baselines/models/SAC_det"
logdir = "src/stable-baselines/logs"
file_dir = "src/stable-baselines/models/SAC_det.zip"
...
class GiniEnv(gym.Env):
    ...
    def __init__(self, grid_size=10, render_mode=None):
        ...
        self.action_space = spaces.Box(low=19.9, high=20, shape=(1,), dtype=np.float32)
    ...
    def step(self, action):
        action = np.clip(action, 20, 20).astype(np.float32) 
...
def train():
    ...
    model.learn(total_timesteps=100000, tb_log_name="SAC_det", callback=TensorboardCallback())
