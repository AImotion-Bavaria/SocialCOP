...
class GiniEnv(gym.Env):
    ...
    def __init__(self, grid_size=5, start=None, render_mode=None):
    ...
    def step(self, action):
        ...
        self.gini_index = calculate_gini(self.observation["received"])
        self.reward = -100*self.gini_index + sum(self.observation["received"]) + sum(self.observation["valuation"]) 
    ...
    def test(self, iterations=100, filedir=file_dir, start=[1,2,3,4,5], model_name=PPO):
        env = DummyVecEnv([lambda: GiniEnv(grid_size=len(start), render_mode='console', start=start)]) 
        #train()

        #test
        #test_env = DummyVecEnv([lambda: GiniEnv(grid_size=5, render_mode='console')])
        model = model_name.load(filedir, env=env)
